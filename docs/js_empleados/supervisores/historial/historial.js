import { cargarHistorial,manejarClickOperacion,guardarActualizacion,mostrarEliminarHistorial,EliminarHistorial} from "../historial/historial_api.js";
...
export function mostrarHistorial() {
  ...
  function goToPage(page) {
    ...
    const campoSegundo = selectCampoSegundo.value;
    const valorSegundo = inputBusquedaSegundo.value.trim();
    const valorSegundoTexto = String(valorSegundo);
    
    const fechaInicioTexto = fechaInicioInput.value;
    const fechaFinTexto = fechaFinInput.value;
    const estadoFinal = estadoSeleccionado; // Usar el estado que estaba seleccionado

    console.log("Ir a página:", { page, campo, valorTexto, campoSegundo, valorSegundoTexto, fechaInicioTexto, fechaFinTexto, estadoFinal });
  ...
  inputBusquedaSegundo.addEventListener("keydown", (e) => {
    if (e.key === "Enter") {
      e.preventDefault();
      aplicarFiltro();
    }
